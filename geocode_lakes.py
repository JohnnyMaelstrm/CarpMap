...
Tarvitsee ajaa uudelleen vain kun lisäät uusia Excel-tiedostoja.
"""

import openpyxl, json, time, urllib.request, urllib.parse, re

# ---- MUOKKAA TÄTÄ LISTAA KUN TULEE UUSIA EXCELEITÄ ----
# (sheet_name=None lukee kaikki välilehdet automaattisesti)
SOURCES = [
    ('Istutukset_Karppi-Siika_Pirkanmaa.xlsx',       'Pirkanmaa',    None),
    ('Istutukset_Siika-Karppi_Varsinais-Suomi.xlsx',  'Varsinais-Suomi', None),
    ('Karppi.xlsx',                                   None,           None),  # lukee kaikki välilehdet
]

def extract_carp(filepath, region_override, sheet_name):
    ...
    for sname, ws in sheets:
        # Käytä välilehden nimeä alueena jos ei ole override
        region = region_override if region_override else sname
        
        for row in ws.iter_rows(min_row=10, max_row=ws.max_row, values_only=True):
            laji = row[15] if len(row) > 15 else None
            if not (laji and 'arppi' in str(laji).lower()):
                continue
...
def geocode_nominatim(nimi, kunta, region):
    ...
    for q in queries:
        ...
        req = urllib.request.Request(url, headers={'User-Agent': 'CarpMap/1.0', 'Accept-Language': 'fi'})
        try:
...
# ---- MAIN ----
all_lakes_dict = {}
for filepath, region, sheet in SOURCES:
    print(f"Luetaan {filepath}...")
    lakes_from_file = extract_carp(filepath, region, sheet)
    print(f"  → {len(lakes_from_file)} järveä")
...
print(f"\nYhteensä {len(all_lakes)} uniikkia järveä.\n")

# Lataa olemassaoleva data (resume-tuki)
try:
    with open('lakes_geocoded.json', 'r', encoding='utf-8') as f:
        existing = {(l['jarvinumero'] + '_' + l['region'] + '_' + l['kunta']): l for l in json.load(f)}
    print(f"Resuming — {sum(1 for l in existing.values() if l['lat'])} koordinaattia jo tallennettu\n")
...
print(f"\nValmis! {ok}/{len(all_lakes)} järveä koordinaateilla.")
if failed:
    print(f"Ei löydetty ({len(failed)}): {', '.join(failed)}")
print("\nSeuraavaksi: python3 geocode_jarvinro.py")